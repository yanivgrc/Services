/* GRC·LABS — cyber screensaver (scene console)
   After 9s idle: a one-time matrix entrance dissolves into a single bash-style
   console. Scenes play inside it, separated by a full-screen matrix wipe (~3s):
     ...
     C) ASCII portrait — occasional; square, gently breathing.
...
(function () {
  ...
  function getScene(cmd) {
    ...
    var rows = [], phase = 'cmd', typed = 0, t = 0, setupIdx = 0, bodyIdx = 0, holdT = 0, blink = 0, elapsed = 0, thinkT = 0, thought = false;
    var cmdStr = 'pip install ' + pkg, setup = buildSetup(cmd.name, pkg);
    function frame(dt) {
      elapsed += dt; blink += dt;
      if (phase === 'cmd') { typed += dt / 52; if (typed >= cmdStr.length) { typed = cmdStr.length; rows.push({ cmd: true, text: cmdStr }); phase = 'setup'; t = 0; } }
      else if (phase === 'setup') { t += dt; var sl = setup[setupIdx], c0 = sl ? sl.charAt(0) : ''; var need = (c0 === '$') ? 420 : ((c0 === '+' || c0 === '|') ? 55 : 150); if (setupIdx < setup.length && t > need) { rows.push(setupRow(sl)); setupIdx++; t = 0; } if (setupIdx >= setup.length) { phase = 'body'; bodyIdx = 0; t = 0; } }
      else if (phase === 'body') {
        if (!thought && cmd.body[bodyIdx] === 'Output:') { phase = 'think'; thinkT = 0; thought = true; }
        else { t += dt; var nl = cmd.body[bodyIdx]; var need2 = (nl !== undefined && isHeader(nl)) ? 520 : 290; if (bodyIdx < cmd.body.length && t > need2) { rows.push({ text: nl, color: colorFor(nl) }); bodyIdx++; t = 0; } if (bodyIdx >= cmd.body.length) { phase = 'resultHold'; t = 0; } }
      }
      ...
      if (elapsed > 42000) phase = 'done';
      var activeCmd = (phase === 'cmd') ? cmdStr.slice(0, Math.floor(typed)) : ((phase === 'resultHold' || phase === 'cta') ? '' : null);
      drawConsole(rows, activeCmd, null, (Math.floor(blink / 530) % 2) === 0, (phase === 'think') ? thinkStr(thinkT) : null);
      return phase === 'done';
    }
    return { frame: frame };
  }
  ...
  function geoScene(shape) {
    var t = 0, REVEAL = 3400, DUR = 5400, gw = 0, gh = 0, fs = 0, x0 = 0, y0 = 0, cw = 0, skey = -1, grid = null, RAMP = ' .:-=+*#%@';
    ...
    return { frame: function (dt) {
      ...
      var pulse = 1 + 0.012 * Math.sin(t * 0.0035);
  ...
  var matrix = (function () {
    ...
    function reset(fromTop) { fs = Math.max(12, Math.min(20, Math.round(W / 70))); cols = Math.ceil(W / fs); rows = Math.ceil(H / fs); drops = []; speeds = []; for (var i = 0; i < cols; i++) { drops[i] = fromTop ? -Math.random() * 4 : Math.floor(Math.random() * -rows); speeds[i] = 0.55 + Math.random() * 0.8; } }
    function draw(dt) {
      ...
      for (var i = 0; i < cols; i++) { var ch = GLYPHS[(Math.random() * GLYPHS.length) | 0], y = drops[i] * fs; ctx.fillStyle = Math.random() > 0.94 ? BRIGHT : GREENS[(Math.random() * GREENS.length) | 0]; if (y > 0) ctx.fillText(ch, i * fs, y); if (y > H && Math.random() > 0.975) { drops[i] = Math.floor(Math.random() * -8); speeds[i] = 0.55 + Math.random() * 0.8; } drops[i] += speeds[i]; }
    }
  ...
  var ENTRANCE_MS = 3500, DISSOLVE_MS = 800, WIPE_MS = 3000;
  ...
  function frame(now) {
    ...
    if (sphase === 'entrance') {
      ...
      if (scene.frame(dt)) { sphase = 'wipe'; pt = 0; matrix.reset(true); }
    } else if (sphase === 'wipe') {
      matrix.draw(dt); pt += dt;
      if (pt > WIPE_MS - 450) { var a2 = Math.min((pt - (WIPE_MS - 450)) / 450, 1); ctx.globalAlpha = a2; ctx.fillStyle = BG; ctx.fillRect(0, 0, W, H); ctx.globalAlpha = 1; }
  ...
  function activate() {
    ...
    if (FORCE_SCENE) { scene = nextScene(); sphase = 'scene'; } else { sphase = 'entrance'; pt = 0; matrix.reset(false); ctx.fillStyle = BG; ctx.fillRect(0, 0, W, H); }
    raf = requestAnimationFrame(frame);
  }
